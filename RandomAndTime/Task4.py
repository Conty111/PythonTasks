...
data = input("Удалить с поля (для завершения - off)? ")

while data != "off":
    timeout = int(input("Длительность удаления с поля: "))
    print(f"Вы удалены с поля на {timeout} минут(-ы)")
    sleep(timeout)
    print("Возрващайтесь на поле!")

    data = input("\nУдалить с поля (для завершения - off)? ")
